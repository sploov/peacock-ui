    ...
    >
      ...
      <motion.div variants={item} className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-8 border-b border-white/5">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-xl bg-peacock-primary/20 flex items-center justify-center text-peacock-primary border border-peacock-primary/20">
          ...
          <div>
            ...
            <p className="text-white/40 text-sm">Real-time interface synthesis engine v3.0.0</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
              ...
              <div className="flex flex-col h-full justify-between">
                <div className="space-y-6">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-white/20 font-mono text-xs">/src/engine/physics.ts</span>
                  </div>
                  <h2 className="text-5xl font-black text-white leading-tight">
                    High Fidelity <br/>
                    Digital <span className="text-peacock-primary">Matter</span>
                  </h2>
                ...
                <div className="pt-12 grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    ...
                    <div key={i} className="p-4 rounded-2xl bg-white/5 border border-white/5 group hover:bg-white/[0.08] transition-colors">
                      <div className="text-peacock-primary mb-2 opacity-50 group-hover:opacity-100 transition-opacity">{stat.icon}</div>
                      <div className="text-xl font-bold text-white">{stat.value}</div>
                      <div className="text-[10px] text-white/30 uppercase font-bold tracking-widest mt-1">{stat.label}</div>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <PeacockTabs 
                ...
                items={[
                  { 
                    id: 'an', 
                    label: 'Analytics', 
                    content: <div className="mt-4 space-y-3">
                      <PeacockSkeleton className="h-4 w-full rounded-md" />
                      <PeacockSkeleton className="h-4 w-2/3 rounded-md opacity-50" />
                      <div className="pt-2 flex gap-2">
                        <div className="h-12 flex-1 bg-peacock-primary/10 rounded-xl border border-peacock-primary/20"></div>
                        <div className="h-12 flex-1 bg-white/5 rounded-xl border border-white/10"></div>
                      </div>
                    </div>
                  },
                  { 
                    id: 'se', 
                    label: 'Security', 
                    content: <div className="mt-4 p-4 rounded-xl bg-peacock-danger/10 border border-peacock-danger/20 text-xs text-peacock-danger">
            ...
            <motion.div variants={item} className="plumage-glass rounded-3xl p-8 border border-white/10 flex flex-col justify-center items-center text-center gap-4">
               ...
               <div>
                 <div className="text-white text-lg font-bold">Quantum Team</div>
                 <div className="text-xs text-white/40 font-mono">Contributor Network</div>
               </div>
               <div className="flex -space-x-3 mt-2">
                  {[1,2,3,4].map(i => (
                    <div key={i} className="w-8 h-8 rounded-full border-2 border-[#0f0f12] bg-peacock-surface-3 flex items-center justify-center text-[10px] text-white font-bold">
                      +{i}
                    </div>
                  ))}
               </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          <motion.div variants={item} className="plumage-glass rounded-3xl p-6 border border-white/10 flex-1 flex flex-col gap-6">
            <h4 className="text-sm font-bold text-white/40 uppercase tracking-[0.2em]">Live Activity</h4>
            ...
            <div className="space-y-6">
              {[
                ...
                <div key={i} className="flex items-start gap-4">
                  ...
                  <div>
                    <div className="text-sm text-white/80"><span className="font-bold text-white">{activity.user}</span> {activity.action}</div>
                    <div className="text-[10px] text-white/30 uppercase font-medium mt-1">{activity.time}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <motion.div variants={item} className="bg-peacock-primary/10 rounded-3xl p-8 border border-peacock-primary/20 relative overflow-hidden group">
            ...
            <h4 className="text-xl font-bold text-white mb-2">Join the Collective</h4>
